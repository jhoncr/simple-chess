...
const StartGameRequest = z.object({
...
export const initGame = onCall({cors: "*", enforceAppCheck: true}, async (request) => {
  ...
  if (!data.gameId && !data.startPiece) {
    ...
    logger.info(data);
    throw new HttpsError("invalid-argument", "Either gameId or startPiece must be provided");
  }
      ...
      if (request.auth.uid in gameData.players) {
        ...
        await gameRef.update({
          [`players.${request.auth.uid}`]: {
            pstatus: "active",
            piece: gameData.players[Object.keys(gameData.players)[0]].piece === "w" ? "b" : "w",
