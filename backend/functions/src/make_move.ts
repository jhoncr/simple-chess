...
const moveSchema = z
  .object({
    ...
    // or empty string for promotion: z.string().optional().
...
export const makeMove = onCall(
  ...
  async (request) => {
    ...
    logger.info(
      `Player ${uid} in game ${parsed.data.gameId}. 
       Move: ${parsed.data.from} to ${parsed.data.to}`
    );
    const game = new Chess(gameData.gameState);
